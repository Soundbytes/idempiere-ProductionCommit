...
public class WQuickADForm extends ADForm implements IQuickForm
{
	...
	private Grid 					north				= new Grid();


	public WQuickADForm(AbstractADWindowContent winContent, boolean m_onlyCurrentRows, int m_onlyCurrentDays)
	{
		super();

		this.setMode(Mode.HIGHLIGHTED);
		this.setSizable(true);
		this.setBorder(true);		
		adWinContent = winContent;

		adWinContent.setStatusBarQF(new StatusBar());
		
		onlyCurrentRows = m_onlyCurrentRows;
		onlyCurrentDays = m_onlyCurrentDays;
		this.gridTab = adWinContent.getADTab().getSelectedGridTab();//.getIncludedTabs().get(0);
	...
	public StatusBar getStatusBar() {
		return adWinContent.getStatusBarQF();
	}
